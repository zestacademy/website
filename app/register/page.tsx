...
import { doc, setDoc, runTransaction, serverTimestamp, Transaction, getDoc } from "firebase/firestore"
...
export default function RegisterPage() {
    ...
    async function onSubmit(event: React.SyntheticEvent) {
        ...
        try {
            ...
            // 2. Generate Zest ID and Store in Firestore
            let zestId = ""
            try {
                await runTransaction(db, async (transaction: Transaction) => {
                    ...
                    // Format: ZU0001, ZU0002, ...
                    zestId = `ZU${newCount.toString().padStart(4, '0')}`
            ...
            try {
                const message = `${email} from Zest acadamy `
                const messagesRef = ref(database, 'registration_logs')
                ...
                console.error("Error logging to RTDB:", rtdbError)
            }

            await sendEmailVerification(user)

            // Set welcome data and show welcome dialog instead of verify dialog
            setWelcomeData({ name: name || user.email?.split('@')[0] || "User", zestId: zestId })
            setShowWelcomeDialog(true)
            // setShowVerifyDialog(true) // We will verify via email but show welcome dialog first
        } catch (e: any) {
            console.error(e)
    ...
    async function handleGoogleLogin() {
        ...
        try {
            ...
            if (!userDoc.exists()) {
                // New Google user - Generate Zest ID and create profile
                let zestId = ""
                try {
                    await runTransaction(db, async (transaction: Transaction) => {
                        const counterRef = doc(db!, "counters", "users")
                    ...
                    // Log to RTDB
                    const message = `${user.email} from Zest acadamy `
                    const messagesRef = ref(database, 'registration_logs')
                    const newMessageRef = push(messagesRef)
                    await set(newMessageRef, {
                        message: message,
                    ...
                    // New user created via Google - Show Welcome Dialog
                    setWelcomeData({ name: user.displayName || "User", zestId: zestId })
                    setShowWelcomeDialog(true)
                    return // Stop execution here to prevent redirect
                } catch (err) {
        ...
        <div className="container relative min-h-screen flex-col items-center justify-center grid lg:max-w-none lg:grid-cols-2 lg:px-0">
                    ...
                    <Card className="border-none shadow-none">
                        <CardContent className="p-0">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <Button variant="outline" disabled={isLoading} onClick={() => console.log("Zest ID register")}>
            ...
            <Dialog open={showWelcomeDialog} onOpenChange={setShowWelcomeDialog}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-center text-xl">Welcome to Zest Academy!</DialogTitle>
                    </DialogHeader>
                    <div className="flex flex-col items-center justify-center space-y-4 py-4">
                        <div className="text-center space-y-2">
                            <p className="text-lg font-semibold">Hello, {welcomeData.name}!</p>
                            <div className="bg-primary/10 text-primary px-4 py-2 rounded-lg font-mono text-xl font-bold tracking-wider">
                                {welcomeData.zestId}
                            </div>
                            <p className="text-sm text-muted-foreground">
                                This is your unique Zest ID.
                            </p>
